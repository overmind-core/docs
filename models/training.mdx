...
The catalog spans current open-weight instruct families such as Qwen, Llama, Gemma, Phi, and Ministral, from sub-1B compact models to 70B-class. The in-product picker is the live source of truth for exactly what's available.
...
- A run is rejected up front, with an explicit message, if your longest row exceeds the chosen model's trainable context. This is another reason the wizard filters the catalog by your token distributions.
