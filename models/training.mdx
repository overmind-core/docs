...
A succeeded run deploys its model automatically; there is no separate publish step. The model appears in [Inference](/models/inference), ready to serve and call, and on the agent's **Models** tab alongside the incumbent. The final weights are downloadable if you want to run the model elsewhere. They're yours. Everything stays linked, so a served model traces back to its training job, its exact dataset commit, and the eval scores that justified it.
