...
The separation between training and eval data is enforced, not advisory. Any training row whose source trace also appears in the eval dataset is excluded from training automatically, so the benchmark never scores data the model saw during training. Within the training data, a validation split (20% by default) is carved out — randomly with trace-safe grouping, or in order.
...
This is the part that makes a Training run conclusive rather than hopeful. Each job spawns judge evaluations at two points:
...
A succeeded run deploys its model automatically; there is no separate publish step. The model appears in [Inference](/models/inference), ready to serve and call, and on the agent's **Models** tab alongside the incumbent. The final weights are downloadable if you want to run the model elsewhere — they're yours. Everything stays linked: a served model traces back to its training job, its exact dataset commit, and the eval scores that justified it.
