...
| **Training dataset** | A **train** dataset whose active version fits (chat transcripts with an assistant turn, the capability's own). See [Datasets](/core/datasets#what-makes-a-version-valid) |
...
The train/eval separation is checked, not assumed: an overlap check compares the two datasets' source traces (`GET /api/finetuning-jobs/dataset-overlap/`), the wizard shows the count, and a job whose training version shares a trace with its eval version is refused until you split them, so the benchmark never scores data the model saw. A **Train + eval** dataset pair is disjoint by construction. The job pins and freezes the exact train, validation and eval versions it was checked on, and validation runs before launch with the same rules the [contract](/core/datasets#what-makes-a-version-valid) applies, so problems surface as fixable findings up front instead of a failed run an hour in.
...
- The run uses a [version](/core/datasets#4-use-a-version); keep curating in the workshop and the next run picks up the new active version.
