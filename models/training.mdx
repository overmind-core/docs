...
The training wizard lists these 38 open-weight models. Modal and Baseten share the same catalog rows. Every trained model serves through the same [OpenAI-compatible API](/models/inference#calling-your-model) and its weights are downloadable. Dense models of 27B and above and every mixture-of-experts model train as LoRA adapters only; the picker shows "LoRA fine-tuning" with no switch for them.
