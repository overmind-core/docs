...
The catalogue spans current open-weight instruct families, from 350M compact models to 35B-class mixture-of-experts, and is listed in full in the [Model Library](https://www.overmindlab.ai/models).
...
```flow
flow row
title: How a training run is decided, from dataset to deployed model
D[FT dataset] -> T[Training run | up to 4 experiments]
E[Eval dataset + rubric] -> B[Benchmark vs baseline]
T -> B
B -> DEC{Fine-tune wins?} : delta per metric
DEC -> DEP[Auto-deployed to Inference] : Yes
DEP -> PR[Model PR switches the agent]
DEC -> IT[Adjust data or tier, rerun] : No
```
