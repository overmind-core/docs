...
The core package is tracing-only. The `overmind` CLI used by [optimisation runs](/latest/agent-testing/optimisers#prerequisites) and the OpenAI-compatible [inference client](/latest/models/inference#calling-your-model) come with the `cli` and `inference` extras.
...
| --- | --- | --- |
| `overmind_api_key` | `str \| None` | Falls back to `OVERMIND_API_KEY`, then the project credential saved by `overmind sync` |
...
Environment variables: `OVERMIND_API_KEY`, `OVERMIND_API_URL`, `OVERMIND_SERVICE_NAME`, `OVERMIND_CAPABILITY_ID`, `OVERMIND_CAPABILITY_NAME`, `OVERMIND_STRICT_MODE`. In a synced local repository, the Python SDK can read the project key, API URL, and project ID from `overmind.toml` plus the ignored `.overmind/credentials.toml` sidecar. Deployments should provide credentials through their runtime secret configuration.
...
| `conversation_id` | Groups this run into a [session](/latest/core/observability#sessions) |
...
When capability sync finds missing telemetry, the setup flow can retrieve exact instrumentation tickets from the capability's behaviour registry. Each ticket names the capability, behaviour, source file and line, required scope, and decorator. Apply those placements verbatim so the resulting spans bind to the synced contracts.
...
How the platform uses them is on [Trace scoring](/latest/agent-testing/trace-scoring#runtime-expectations-from-the-sdk).
...
| Helper | Effect |
| --- | --- |
| `set_conversation_id(id)` | Group traces into a [session](/latest/core/observability#sessions) |
