...
The package also ships the `overmind` CLI used by [optimisation runs](/latest/agent-testing/optimisers#prerequisites) and an OpenAI-compatible [inference client](/latest/models/inference#calling-your-model).
...
| Helper | Effect |
| --- | --- |
| `set_agent_id(id)` / `set_agent_name(name)` | Bind downstream spans to a [capability](/latest/core/capabilities) |
| `set_project_id(id)` | Set the project explicitly |
| `set_conversation_id(id)` | Group traces into a [session](/latest/core/observability#sessions) (`conversation` is the decorator form) |
