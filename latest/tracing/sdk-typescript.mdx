...
| Helper | Effect |
| --- | --- |
| `setAgentId(id)` / `setAgentName(name)` | Bind downstream spans to a [capability](/latest/core/capabilities) |
| `setProjectId(id)` | Set the project explicitly |
| `setConversationId(id)` | Group traces into a [session](/latest/core/observability#sessions) |
