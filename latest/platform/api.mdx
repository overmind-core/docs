...
| Agent graph | `/api/agent/` | one call: current capabilities, edges, last sync |
...
| Datasets | `/api/datasets/` | create (`intent`, `capability` optional; `source` = `upload_id`, `rows`, `text`, or `traces`), read (cells with versions, chat), patch (`name`, `intent`, `capability`, `active`; intent and capability freeze once a version is used), delete; `POST .../trace-count/`; `GET .../rows/` (paged, `?cell=`, `?diff=1` for marks, sort/filter/search), `GET .../rows/{index}/`, `GET .../columns/`, `POST .../query/` (read-only SQL over `t`), `POST .../cells/` (add), `PATCH|DELETE .../cells/{id}/`, `POST .../cells/{id}/accept/` (run a proposal), `.../usage/`, `POST .../run/`, `POST .../chat/`, `GET .../export/?fmt=jsonl|csv&cell=` (a raw stream, not a use), `GET .../events/` (SSE), `POST .../generate-evals/`, `POST .../context/refresh/` — see [Building a dataset](#building-a-dataset) |
...
| Resource | Base path | Custom actions |
| --- | --- | --- |
| Optimiser runs | `/api/optimizer-experiments/` | `GET .../iterations/`, `GET .../commands/`, `POST .../cancel/`, `GET connection/` |
| Training runs | `/api/finetuning-jobs/` | `POST validate-dataset/`, `POST recommend/`, `POST estimate/`, `GET models/`, `GET dataset-overlap/`, `GET .../loss-curves/`, `GET .../events/`, `POST .../cancel/`, `POST .../retry/`, `GET .../model-swap-prompt/` |
...
| Endpoint | Purpose |
| --- | --- |
| `POST /api/v1/traces` (alias `/v1/traces`) | [OTLP trace ingest](/latest/core/observability#the-otlp-endpoint) — protobuf, not JSON |
| `POST /api/v1/chat/completions`, `GET /api/v1/models` | [OpenAI-compatible inference](/latest/models/inference#endpoints) |
...
<Steps>
  <Step title="Land a source">
    ...
    `trace` or `turn`; `POST .../trace-count/` gives the row count per grain; an interrupted trace with no root span counts and lands like any other). The response is
