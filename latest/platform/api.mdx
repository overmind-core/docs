...
| Datasets | `/api/datasets/` | create (`intent`, `capability` optional; `source` = `upload_id`, `rows`, `text`, or `traces`), read (cells with versions, chat), patch (`name`, `intent`, `capability`, `active`; intent and capability freeze once a version is used), delete; `GET .../rows/` (paged, `?cell=`, `?diff=1` for marks, sort/filter/search), `GET .../rows/{index}/`, `GET .../columns/`, `POST .../query/` (read-only SQL over `t`), `POST .../cells/` (add), `PATCH|DELETE .../cells/{id}/`, `POST .../cells/{id}/accept/` (run a proposal), `.../usage/`, `POST .../run/`, `POST .../chat/`, `GET .../export/?fmt=jsonl|csv&cell=` (a raw stream, not a use), `GET .../events/` (SSE), `POST .../generate-evals/`, `POST .../context/refresh/` — see [Building a dataset](#building-a-dataset) |
...
<Steps>
  <Step title="Land a source">
    ...
    pasted data, or `traces` (a trace-list selection or `trace_ids`; one row lands per trace,
    and an interrupted trace with no root span lands like any other). The response is
