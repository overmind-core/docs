...
Every trace that reaches Overmind is scored against the owning capability's active [eval set](/latest/agent-testing/eval#eval-sets). Scoring does not grade the trace as one blob: it carves the trace into **units**, binds each unit to one of the capability's **tasks**, judges each unit on its own, and rolls the results up into an **execution score** per unit and a **session score** per conversation. This page is the contract behind the **Task executions** view in [Observability](/latest/core/observability#exploring-traces-in-the-console).
...
A **task** is one behaviour a capability performs, minted by [capability sync](/latest/core/capabilities#local-discovery) from the trajectory map: an entry anchor, the ordered anchor segments that follow it, its tool set, and its terminal. Tasks keep their identity across syncs through the entry anchor.
...
The eval set's **trace scoring** members run per unit. Every evaluator carries a [claim](/latest/agent-testing/eval#claims) — a type and a grain — and the claim decides how its verdict composes:
...
See the [Python SDK](/latest/tracing/sdk-python#runtime-expectations) for the calls.
