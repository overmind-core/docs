...
```flow
flow row
title: The Overmind loop, from agent discovery through to served inference
A[Agents | discovered from code + telemetry] -> O[Observability | traces, scored on arrival]
O -> D[Datasets | curated in the Workshop]
D -> E[Eval | what good means, measured]
E -> OP[Optimisers | code changes as PRs]
E -> T[Training | fine-tunes vs the baseline]
T -> I[Inference | served, OpenAI API compatible]
OP ~> A : merged changes run in prod
I ~> O : calls are traced
```
