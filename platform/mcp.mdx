...
MCP is Overmind's first-class agent surface: a project-scoped [Model Context Protocol](https://modelcontextprotocol.io) server at `/api/mcp/` with the same reach as the Console, not a thin wrapper over the REST API. A coding agent connected to it can inspect health and failures, land and shape datasets, author evaluators and run evals, launch and follow training, schedule optimisation runs, and wire up connectors and instrumentation — everything except the steps that need your filesystem, which it hands to the [CLI](/platform/cli).
