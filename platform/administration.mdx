...
Managed under **Observability → Integrations**. Four connectors are available: **Langfuse** (host URL + `pk-lf-...` / `sk-lf-...` keys, per-project import), **LangSmith** (API key `lsv2_sk_...` or `lsv2_pt_...`; API URL optional, EU is `eu.api.smith.langchain.com`), **Braintrust** (API key or service token `bt-st-...`; API URL), and **Galileo** (API key; API URL). Each runs a resumable backfill then incremental sync, with a manual **Sync now**. Credentials are encrypted at rest and used only to read traces. A coding agent configures and syncs the same connectors through MCP once the credential exists.
