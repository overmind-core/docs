...
| Datasets | `/api/datasets/` | create (`target` = `train` or `eval`; `source` = `upload_id`, `rows`, `text`, `traces`, or `checkpoint_id`), read, patch (`name`, `active_cell_id`; target and capability are fixed), delete; `GET .../rows/` (paged, `?source=`, `?cell=`, `?checkpoint=`, sort/filter/search), `GET .../rows/{index}/`, `GET .../columns/`, `POST .../query/` (read-only SQL), `PUT .../cells/` (save, `run: true` to execute), `POST .../run/`, `POST .../activate/`, `POST .../fork/`, `GET .../checkpoints/`, `GET .../checkpoints/{id}/` (cell, delta, contract, alignment, columns, stats), `.../diff/{other}/`, `.../usage/`, `.../delete/`, `POST .../append/`, `POST .../refresh/`, `GET .../export/?fmt=jsonl|csv|training`, `GET .../events/` (SSE), `POST .../generate-evals/`, `POST .../context/refresh/` — see [Building a dataset](#building-a-dataset) |
...
<Steps>
  <Step title="Land a source">
    ...
    `POST /api/datasets/` with `project`, `name`, `target` (`train` or `eval`), the
  ...
  <Step title="Shape it">
    ...
    `POST .../activate/` with `cell_id` moves that pointer. `GET .../events/` streams
