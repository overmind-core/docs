...
| Datasets | `/api/datasets/` | create (`intent`, `capability` optional; `source` = `upload_id`, `rows`, `text`, or `traces`), read (cells with versions, chat), patch (`name`, `intent`, `capability`, `active`; intent and capability freeze once a version is used), delete; `POST .../trace-count/`; `GET .../rows/` (paged, `?cell=`, `?diff=1` for marks, sort/filter/search), `GET .../rows/{index}/`, `GET .../columns/`, `POST .../query/` (read-only SQL over `t`), `POST .../cells/` (add), `PATCH|DELETE .../cells/{id}/`, `POST .../cells/{id}/accept/` (run a proposal), `GET .../cells/{id}/diff/`, `.../usage/`, `POST .../run/`, `POST .../chat/`, `GET .../export/?fmt=jsonl|csv|training&cell=` (a use), `GET .../events/` (SSE), `POST .../generate-evals/`, `POST .../context/refresh/` — see [Building a dataset](#building-a-dataset) |
...
<Steps>
  ...
  <Step title="Shape it">
    ...
    `chat_delta` and `chat_cell` as server-sent events.
  </Step>
  <Step title="Read a version">
    The dataset's `cells` carry every version: `version`, `state`, `rows`, `columns`,
    `intent_report` and `capability_report`, `fits` against the intent, the `diff` against the
    cell before, `stats`, and `frozen`. `GET .../rows/?cell=&diff=1` pages a frame with
    `marks` (rows added, values changed with the old value); `GET .../cells/{id}/diff/`
    gives the summary, examples and the removed rows; `.../usage/` lists the runs that used it.
  </Step>
  <Step title="Use it">
