...
| Conversations | `/api/conversations/` | judged turns grouped by `conversation.id`, with verdict and asks; `GET counts/` per verdict state, `GET thread/?key=` for one conversation |
| Task executions | `/api/task-executions/` | one judged turn per row: route, step results, scores |
