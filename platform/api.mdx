...
| Method | How | Use for |
| --- | --- | --- |
| **API key** | `X-Api-Key: ovr_...`, `Authorization: Bearer ovr_...`, or `Authorization: Api-Key ovr_...` | SDKs, OTLP ingest, scripts, CI, the CLI, MCP, inference, `overmind scan push` |
...
| Scan | `POST /api/v1/scan/` | what `overmind scan push` sends: `{skeleton, manifest, report, cards}`; API key only; `413` over 20 MB, `400 invalid_scan` when the skeleton hash, manifest or a card fails validation, `400` when `manifest.project.id` is missing and the key sees more than one project, `403` when the key does not match it; returns `{project_id, snapshot_id, capabilities: [{key, id, status, created}]}` |
| Latest scan | `GET /api/projects/{project_id}/scan/latest/` | the last push: report, skeleton counts and one row per cluster with the capability it carried; `404 no_scan` until a push lands |
