...
| Datasets | `/api/datasets/` | create (`target` = `train` or `eval`; `source` = `upload_id`, `rows`, `text`, `traces`, or `checkpoint_id`), read, patch (`name`, `capability`, `target`, `active_cell_id`), delete; `GET .../rows/` (paged, `?source=`, `?cell=`, `?checkpoint=`, sort/filter/search), `GET .../rows/{index}/`, `GET .../columns/`, `POST .../query/` (read-only SQL), `PUT .../cells/` (save, `run: true` to execute), `POST .../run/`, `POST .../activate/`, `POST .../pending/` (`action`: `confirm`, `undo`, `accept`, `reject` + `cell_id`), `POST .../fork/`, `GET .../checkpoints/`, `GET .../checkpoints/{id}/` (cell, delta, contract, alignment, columns, stats), `.../diff/{other}/`, `.../usage/`, `.../delete/`, `POST .../append/`, `POST .../refresh/`, `GET .../export/?fmt=jsonl|csv|training`, `GET .../events/` (SSE), `POST .../generate-evals/`, `POST .../context/refresh/` — see [Building a dataset](#building-a-dataset) |
...
A dataset lands its rows as-is with a target and shapes them in a notebook; a run reads the product checkpoint. The same sequence backs the console and Brain's dataset tools.

<Steps>
  <Step title="Land a source">
    A file goes up in chunks (`POST /api/uploads/`, then `PUT /api/uploads/{id}/chunk/?offset=`).
    `POST /api/datasets/` with `project`, `name`, `target` (`train` or `eval`), an optional
    `capability`, and exactly one `source`: `upload_id` (+ `filename`), `rows` or `text` for
    pasted data, `traces` (a trace-list selection or `trace_ids`, plus `grain`: `task` or
    `trace`), or `checkpoint_id` to start from another dataset's checkpoint. The response is the dataset;
  ...
  <Step title="Shape it">
    ...
    `POST .../run/` runs them later. Every run writes a checkpoint per cell (an unchanged
    prefix reuses its checkpoints) and points `product` at the active cell's;
    `POST .../activate/` with `cell_id` moves that pointer. A write made by a chat turn is
    staged: the dataset's `pending` block lists the changed cells, and `POST .../pending/`
    settles it (`confirm`, `undo`, or `accept` / `reject` one `cell_id`). `GET .../events/` streams
    `cell_started`, `cell_repairing`, `cell_done`, `cell_failed`, `run_done` and `run_failed`
    as server-sent events, each with its `checkpoint_id`.
  </Step>
  <Step title="Read a checkpoint">
    `GET .../checkpoints/{id}/` returns the cell snapshot with its delta, `contract`
    (`train`, `eval` or `raw`) with the `contract_report` behind it, `meets_target`, the
    `alignment` report when a capability is bound, the column manifest and `stats`.
    `.../diff/{other}/` compares two checkpoints; `.../usage/` lists the runs that pinned one;
    `POST .../fork/` starts a new dataset from one.
  </Step>
  <Step title="Pin it">
    Eval runs, optimiser experiments and finetuning jobs take `dataset` and, optionally,
    `checkpoint`; without it the product is pinned. A product whose contract does not fit,
    or a checkpoint of another dataset, is refused with the reason.
  </Step>
  <Step title="Pull it">
    A local loop reads the pinned checkpoint, never the dataset: `GET .../export/?fmt=jsonl&checkpoint={id}`.
    The response carries `X-Overmind-Checkpoint` and `X-Overmind-Product-Hash`; the SDK
    caches the file as `.overmind/datasets/{checkpoint}.jsonl` with the hash beside it and fetches
    again when the hash no longer matches the pin.
  </Step>
</Steps>
