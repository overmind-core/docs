...
| Traces | `/api/traces/` | list returns root spans with `trace_status` (`live`, `completed`, `interrupted`) and the `feedback_score.trace_scoring` block; `GET /{trace_id}/` returns all of a trace's spans; `GET services/`, `GET models/` |
| Sessions | `/api/sessions/` | traces grouped by `conversation.id`, with session scores |
| Task executions | `/api/task-executions/` | one row per scored unit — see [Trace scoring](/agent-testing/trace-scoring#reading-executions-over-the-api) for the filters; `GET /{id}/` adds the observed route, step results and intent |
| Tasks | `/api/behaviours/` | list (execution counts, mean score); `GET coverage/?capability=` (eval-set coverage per task); `GET /{id}/evaluators/` |
