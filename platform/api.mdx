...
| Capabilities | `/api/capabilities/` | `GET .../eval_spec/`, `GET .../prompts/`, `GET .../merge-preview/`, `POST .../merge/`, `GET .../split-preview/`, `POST .../split/`, `POST .../retire/`, `POST .../reactivate/` |
| Agent graph | `/api/agent/` | one call: current capabilities, edges, history, last scan |
...
sequence backs the console's upload dialog and the agent's dataset tools.
...
<Steps>
  <Step title="Stage a source">
    ...
    `intent` (`eval` or `ft`) and, for traces, the `surface` (`agent` or `model`).
