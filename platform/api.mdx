...
<Steps>
  ...
  <Step title="Pull it">
    A local loop reads the pinned version, never the dataset: `GET .../export/?fmt=jsonl&version={id}`.
    The response carries `X-Overmind-Dataset-Version` and `X-Overmind-Product-Hash`; the SDK
    caches the file as `.overmind/datasets/{version}.jsonl` with the hash beside it and fetches
