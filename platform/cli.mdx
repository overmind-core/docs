---
title: CLI
description: The overmind command — init, sync, chassis, dataset upload and export, the optimise loop, model checkpoints, and skills.
---

The `overmind` command ships with the Python package. It is the local half of every workflow that touches your filesystem: it installs the skill and MCP entry, syncs `overmind.toml`, lands files as datasets, drives optimisation runs in your repository, and downloads weights. The MCP server never edits a repository or moves bytes; the CLI does.
