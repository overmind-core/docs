...
A run prepares its samples first — generating outputs for the variants that need them — then scores every sample against every evaluator, then aggregates the summary. Runs move through pending → running → completed (or failed/cancelled) and can be cancelled mid-flight.
...
A sample that exceeds the generation time limit records an error and is excluded from scoring. Scoring starts after sample preparation finishes, so a run can be generating while its score count is still zero.
