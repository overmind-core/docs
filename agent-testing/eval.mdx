...
```flow
flow row
title: What an eval run takes in, and the report it produces
DS[Data source | dataset commit or trace filter] -> R[Eval run]
V1[Variant A, the baseline | e.g. production model] -> R
V2[Variant B | e.g. fine-tune or new prompt] -> R
E[Evaluators | frozen snapshots] -> R
R -> REP[Report | per-variant scores, deltas vs baseline, per-example grid]
```
