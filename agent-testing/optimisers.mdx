...
R -> SC[Score with the eval set | on the platform]
...
- **Plateaus point at the model.** When several runs on a capability stop improving, the run page suggests [Training](/models/training) — by then you already have the dataset and eval set it needs.
