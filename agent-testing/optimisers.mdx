...
<Steps>
  ...
  <Step title="An eval dataset">
    The run replays the agent over the rows of an eval dataset's active version, which must fit both **eval** contracts; the inputs must be a shape the agent can be invoked with. See [Datasets](/core/datasets#what-makes-a-version-valid).
  </Step>
...
| **model_comparison** (Backtesting) | One to five OpenRouter model ids, replayed with the repository's provider calls rewritten to OpenRouter (`overmind.backtest.rewrite_repo`) | Every model, always — a comparison owes a full table |
