...
<Steps>
  ...
  <Step title="An eval dataset">
    The run replays the agent over the rows of an eval dataset's active version, which must fit both **eval** contracts; the inputs must be a shape the agent can be invoked with. See [Datasets](/core/datasets#what-valid-rows-look-like).
  </Step>
