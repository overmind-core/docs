...
Runs can be canceled at any point, and the executioner's status is always visible. If the local CLI disconnects mid-run, you'll see it here instead of wondering why progress stopped.
...
- The eval set is pinned at creation, but the *dataset* defines the situations explored. A narrow dataset produces improvements narrow to match, so add fresh traces to it periodically and optimization is always tested against current traffic.
