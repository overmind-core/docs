...
Scoring is queued the moment a trace's root span lands. A trace is **live** while spans are still arriving or while it has no root span yet, **completed** once the root span has landed, and **interrupted** when no root arrives within the settle window (`TRACE_SETTLE_SECONDS`, default 600 s — a killed run never exports its root, because the root ends last). Nothing is left unscored: a trace the first pass missed — an interrupted one included — is picked up and retried without you asking. The trace list and the trace header show the status.
...
A unit is the slice of a trace that gets its own verdicts. The carve follows a precedence order; the first tier that applies wins:
...
- Every verdict feeds one of three phases by grain — **steps** (unit claims, weight 0.25), **trajectory** (0.25), and **output** (terminal claims, 0.50) — and an empty phase's weight is redistributed to the others. A [session score](#session-scores) then folds a conversation's turns together.

Each verdict is stored on its own: evaluator, target span, typed outcome, score, rationale, and a flat `metadata` block (`passed`, `scope`, `grain`, `gate`, `surface_area`, `sub_scores`). Verdicts are the source of truth — every detail surface reads them over `GET /api/verdicts/`. The unit's span keeps only the composed markers, under `feedback_score.trace_scoring`:
...
Outcomes are typed — `scored`, `abstained`, `not_applicable`, `error` — and only `scored` verdicts feed the composite. A member whose evidence the trace cannot supply abstains rather than guessing; a member skipped for its grain, a behaviour filter, or an interrupted run records a `not_applicable` verdict whose `unmet` list carries a `skip:` clause.
