...
<CardGroup cols={2}>
  ...
  <Card title="Starter evaluators" icon="list-check">
    A first set of evaluators for this specific agent, derived from its purpose, tools, and failure modes, and saved automatically, not held for approval. Review, edit, or archive them in [Eval](/agent-testing/eval) like any other evaluator.
  </Card>
</CardGroup>
...
A row at the top of the tab shows the agent's key facts at a glance: **Model**, **Source**, **Built from**, and **Trace coverage**. Overmind calls the production model the **incumbent**, and it becomes the baseline that fine-tuned models are later measured against. Below that sit the system prompt card, the flow graph, and the components explorer. This tab is the fastest way to sanity-check that Overmind read your code correctly, and the first place to look when an agent is unfamiliar.
...
The console can also warn you when the CLI version last seen from your environment is behind, which matters for [optimisation runs](/agent-testing/optimisers) that execute locally.
