...
That picture is the reference point for the rest of the platform. Traces attach to an agent. Datasets are checked for fit against an agent. Evaluators are scoped to an agent's job. Optimization runs edit an agent's code, and fine-tuned models are trained to replace an agent's production model. The agent page is the best place to see how the pieces fit.

## Why agents are modeled from code

Traces tell you what your agent did. Only the code tells you what it *could do*: which tools exist, which branches never fire, what the output contract is supposed to be. Overmind reads both sides and joins them, static structure from your repository with runtime behavior from your telemetry. Comparing the two often surfaces problems: a tool that's defined but never called, or a code path production traffic never hits.
