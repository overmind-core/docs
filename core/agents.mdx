...
This tab holds the agent's evaluators and its **eval sets** — named groupings of evaluators that define what "good" means for this agent. The **Optimize** button lives here too, because starting an optimization only makes sense once the agent has a way to be judged. See [Eval](/agent-testing/eval) for the full model.
