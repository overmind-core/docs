...
An agent in Overmind is a model of an agent you already run — the support triager, the lead qualifier, the document extractor that lives in your repository and serves real traffic. You don't build agents inside Overmind. Overmind discovers them, reads how they work, and keeps a live picture of them: the prompts, the tools, the control flow, and the behavior they actually exhibit in production.
...
A pile of traces tells you what your agent *did*. Only the code tells you what it *could do* — which tools exist, which branches never fire, what the output contract is supposed to be. Overmind reads both sides and joins them: static structure from your repository, runtime behavior from your telemetry. The gap between the two is often where the interesting problems live, such as a tool that is defined but never called, or a code path production traffic never exercises.
...
<Steps>
  <Step title="Connect GitHub">
    For a private repository, install the Overmind GitHub App from **Settings**. During installation you choose exactly which repositories Overmind can see — it never gets blanket access. For a public repository, paste its URL instead; no GitHub account connection is needed.
  </Step>
  ...
  <Step title="Let the scan run">
    Overmind clones the branch and reads the codebase. The status updates as it works, and a live label tells you what it is doing — searching for agents, mapping tools, and so on. If something fails, you get a short explanation and can run the scan again.
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="Policy" icon="scale-balanced">
    A plain-language description of how the agent is supposed to behave, drafted from the code and editable by you. This is the reference for what counts as correct downstream.
  </Card>
...
The facts bar summarizes the agent's identity and configuration at a glance, including the model it runs in production — Overmind calls this the **incumbent**, and it becomes the baseline that fine-tuned models are later measured against. Below it sit the system prompt card, the flow graph, and the components explorer. This tab is the fastest way to sanity-check that Overmind read your code correctly, and the first place to look when an agent is unfamiliar.

{/* TODO(screenshot): Agent Overview tab with the flow graph and components explorer */}

### Dataset

Every agent can hold several datasets, with one marked **active**. The active dataset is a meaningful pointer: it is what the Optimizer replays the agent against by default, and what run wizards preselect. Building and curating datasets is covered in [Datasets](/core/datasets).

### Eval metrics

This tab holds the agent's evaluators and its **eval sets** — named groupings of evaluators that define what "good" means for this agent. As with datasets, one eval set is active, and the active set is what scores optimization runs and live production traces. The **Optimize** button lives here too, because starting an optimization only makes sense once the agent has a way to be judged. See [Eval](/agent-testing/eval) for the full model.
...
## The two active pointers

An agent's **active dataset** and **active eval set** deserve emphasis, because they quietly drive most default behavior on the platform:

| Pointer | What reads it |
| --- | --- |
| Active dataset | Optimization runs replay against it; training and eval wizards preselect it |
| Active eval set | Scores optimization candidates, gates fine-tunes, and scores live traces as they arrive |

Changing an active pointer changes what future runs measure — in-flight runs pin the versions they started with, so results stay reproducible. Treat switching the active dataset or eval set as a deliberate act, the same way you'd treat changing a CI baseline.

## Keeping the picture current

Overmind remembers the commit it last scanned and compares it against the head of your branch. When the branch moves on, the agent shows as out of sync and offers a re-scan, which rebuilds the discovered structure from current code. You'll also be told if the branch was deleted or the repository became unreachable.

Telemetry needs no such refresh — traces attach continuously, so the runtime half of the picture stays current on its own. The console can also warn you when the CLI version last seen from your environment is behind, which matters for [optimization runs](/agent-testing/optimizers) that execute locally.
...
Most teams follow roughly the same arc. First they connect a repository and check the flow graph against reality — this usually takes minutes and occasionally surfaces a surprise about their own code. Then they turn on tracing so the agent's page fills with production behavior. From there the order varies: some teams go straight to curating a dataset from traces, others start by reviewing the proposed evaluation criteria. Either way, the agent page becomes the home base — the place where data, metrics, runs, and models for one agent stay in one view.
