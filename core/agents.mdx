...
The scan also produces an evaluation spec for the agent: its input schema, expected output fields, tool configuration, and consistency rules, along with which parts of the agent are **optimisable** and which are **fixed**. That boundary is what keeps an optimisation run inside the lines you set. The Optimiser may rewrite an optimisable prompt, but it will not touch an element you've marked fixed.
...
This tab holds the agent's evaluators and its **eval sets**, the named groupings of evaluators that define what "good" means for this agent. The **Optimise** button lives here too, because starting an optimisation only makes sense once the agent has a way to be judged. See [Eval](/agent-testing/eval) for the full model.
