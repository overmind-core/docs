...
```flow
flow row
title: A dataset's edit history, where restoring is itself a new commit
A[Seed] -> B[Fix: dedupe rows] -> C[Edit: relabel 12 rows]
C -> D[Fix: redact PII] -> E[Restore to Seed]
E -> F[...] : new commit, history intact
```
