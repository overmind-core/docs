...
| --- | --- | --- |
| **Eval** | `input` + optional `expected_output` | [Eval runs](/agent-testing/eval), the [Optimiser](/agent-testing/optimisers) |
...
<Tabs>
  ...
  <Tab title="Upload">
    Drag a file anywhere onto the Datasets page, or use the create dialogue. CSV, TSV, JSON, and JSONL are accepted. Uploads are the usual path for seed sets, labelled ground truth, and data exported from another system.
  </Tab>
  <Tab title="From a connector">
    Traces synced from a connected observability tool (Langfuse, LangSmith, and others; see [Observability](/core/observability#importing-from-observability-tools)) land as traces first, then follow the same select-and-create path as native traces.
  </Tab>
</Tabs>
...
Running an analysis produces a report, a score out of 100, and a readiness verdict: **Ready**, **Ready with warnings**, or **Not ready**. The scoring lens follows the dataset's intent. An eval dataset is judged on eval compatibility, a fine-tuning dataset on training compatibility, and an unstructured one on general data quality. The analysis combines two passes: deterministic checks that run in seconds, and an AI analyst pass that reads the corpus for the problems rules can't express, such as label conflicts, answer leakage, or degenerate outputs.
...
Some checks block readiness outright, holding the verdict at Not ready until the problem is fixed or the check is deliberately waived; others only lower the score without blocking anything. Any check can be **waived** in one click when you've judged the finding acceptable for your use case. The waiver records who accepted it and stays visible in a muted style even after the underlying check starts passing, so the decision trail survives.

The safety pillar includes personal-data detection: names, locations, organisations, emails, and phone numbers are identified and can be masked, so what leaves the Workshop is safe to train on.
...
Every dataset carries a commit log from its initial seed onward. Each commit records what kind of change it was (seed, fix, restore, or manual edit), how many rows were added, removed, or modified, a compact row diff against its parent, and the score and verdict at that point. The history therefore doubles as a score timeline. You can restore an older state at any time; a restore creates a new commit on top instead of rewriting history, so nothing is ever lost. To compare against a different dataset of the same intent rather than an earlier commit of this one, use Compare.
...
- **Eval** runs draw their rows from an eval-intent dataset and pin the commit they ran against. See [Eval](/agent-testing/eval).
- **Optimisation** replays your agent over the dataset's datapoints, so the dataset defines the situations the Optimiser is judged on. See [Optimisers](/agent-testing/optimisers).
...
- **Observability** is the upstream supply: keep tracing on and you always have fresh, real examples to add. See [Observability](/core/observability).

## Getting the most out of it

Start small and real. Ten to fifty representative examples taken from traces beat a thousand synthetic rows for almost every purpose except pre-training. Let the Workshop's verdict guide what to fix first (whatever's blocking readiness, then high-severity insights), and waive consciously. When the same defect keeps appearing in fresh trace batches, that's usually a signal about the agent itself, not the data.
