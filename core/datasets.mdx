...
- **Training** consumes fine-tuning-intent datasets, validates them with the same checks at ingest and at training time, and excludes any row whose source trace also appears in your chosen eval dataset. That contamination guard is what keeps benchmarks honest. See [Training](/models/training).
