...
Start small and real. Ten to fifty representative examples taken from traces beat a thousand synthetic rows for almost every purpose except pre-training. Let the Workshop's verdict guide what to fix first (hard gates, then high-severity insights), and waive consciously. When the same defect keeps appearing in fresh trace batches, that's usually a signal about the agent itself, not the data.
