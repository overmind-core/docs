...
Run `overmind scan push` first. It saves your project credential to `.overmind/credentials.toml`, which the Python SDK picks up automatically — no key in your code. Deployed processes get `OVERMIND_API_KEY` through their usual secret configuration.
...
<Tabs>
  ...
  <Tab title="Traces you already have">
    Import from **Langfuse**, **LangSmith**, **Braintrust** or **Galileo**. Add the provider's credentials under **Observability → Integrations**, pick the source project and which capability its traces belong to, and Overmind backfills your history and then keeps polling for new traffic. **Sync now** pulls on demand.
    ...
    Imported traces are scored on arrival and behave like native ones. Credentials are encrypted and only ever used to read. Your coding agent can drive the whole setup through MCP — see [MCP](/platform/mcp).
  </Tab>
</Tabs>
