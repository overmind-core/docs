...
<Warning>
The TypeScript SDK still stamps the retired `overmind.agent.*` identity attributes. The platform now binds spans by `overmind.capability.id` only, so traces from this package stay **unbound** until a release adopts the new attribute. Until then, prefer the [Python SDK](/tracing/sdk-python) or raw OTel with `overmind.capability.id`.
</Warning>
...
| Helper | Effect |
| --- | --- |
| `setAgentId(id)` / `setAgentName(name)` | Bind downstream spans to a [capability](/core/capabilities) |
