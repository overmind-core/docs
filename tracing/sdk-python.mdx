...
`@observe` is the general decorator; `@entry_point`, `@workflow`, `@tool`, `@retrieval`, and `@function` are typed shortcuts. All capture arguments as `inputs` and the return value as `outputs` (JSON-serialised, best-effort), record duration and status, re-raise exceptions after recording them, and work on sync and async functions alike.
...
For functions that handle secrets or oversized payloads, `observe_safe` traces without capturing inputs/outputs.

### `start_span()` — blocks and loops

A context manager for regions that aren't whole functions. Opens a child span under whatever span is active; unlike the decorators it captures nothing automatically — attach metadata via `attributes` or `set_tag()`.
...
Use the same `id` for logically identical prompts across providers and versions and Overmind groups them as one agent. One `PromptString` per LLM call — the SDK raises if it detects more.
