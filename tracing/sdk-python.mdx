...
| `agent_id` | `str \| None` | Capability UUID; stamped as `overmind.agent.id` on every span |
| `agent_name` | `str \| None` | Stable capability name; stamped as `overmind.agent.name` |
...
Environment variables: `OVERMIND_API_KEY`, `OVERMIND_API_URL`, `OVERMIND_SERVICE_NAME`, `OVERMIND_AGENT_NAME`, `OVERMIND_AGENT_ID`, `OVERMIND_STRICT_MODE`.
...
| `capability` / `capability_id` | The capability this run belongs to; falls back to `init()` / `OVERMIND_AGENT_NAME` / `OVERMIND_AGENT_ID` |
...
| `capability` | Scope this span and its children to a named capability (see below) |
| `type` | A `SpanType` or its name (`"tool"`, `"llm"`, `"entry_point"`) |

Captured payloads are scrubbed automatically: secret-named keys are redacted and base64 or data-URL blobs replaced; text is kept in full. `observe_safe` is `observe(capture="none")` for code that handles credentials — prefer masking values before they reach traced functions.
...
`capability(name, id=None)` declares that all work inside belongs to the named capability: every span created inside carries `overmind.agent.name` / `.id`, and the outer identity is restored on exit. Entering a different capability mid-trace is a **handoff** — the first span of the new scope is stamped as a turn, so the platform scores it as a new unit against that capability's eval set. Only declared identities are stamped; nothing is created.
